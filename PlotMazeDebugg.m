function figH = PlotMazeDebugg(fig, mazeSize, walls, targetCell, holes, resetCell, P,stateSpace,controlSpace,startStateIdx,controlInputIdx)
...
for x_i = startStateIdx
    for y_i = 1:size(P,2)
        ...
        %     end
            ...
            if shouldPlotCost
                text( y( 1 ) - 0.9, y( 2 ) - 0.1, num2str( P(x_i,y_i,u_i), 3 ), 'FontSize', 8 );
            end
            if shouldPlotArrow
                center = [ x( 1 ) - 0.5, x( 2 ) - 0.5 ];
                u = controlSpace( u_i, : );

                if sum( abs( u ) ) > 0
                    startPt = center + 0.2 * u / norm( u );
                    endPt = center + 0.5 * u / norm( u );
                    steps = max(abs(u));
